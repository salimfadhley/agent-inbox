...
There is one prompt now and it is generated. `src/agent_inbox/prompts.py` is the only
...
binary was unified into `agent-inbox mcp`.
...
`agent-inbox.toml`. What a role *means* is fetched from the hub when the agent connects,
