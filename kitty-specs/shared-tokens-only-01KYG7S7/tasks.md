...
| WP03 | One screen that lists tokens, not agents | WP02 | Yes, and it is the one an operator sees |
...
| T004 | Minting takes a label and never an actor; legacy bound rows keep working | WP01 | |
...
| T020 | `doctor`'s `_token_help` stops sending operators to a page that is gone | WP04 | |
