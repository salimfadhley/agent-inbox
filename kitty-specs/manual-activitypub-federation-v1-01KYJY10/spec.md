...
|---|---|---|
| FR-001 | Add a Federation UI tab for peer management, federation mode, blocklist, server profile, delivery status, and warnings. | proposed |
...
| FR-013 | Direct federation requires a stable `federation_base_url`. It must be configured before federation can be enabled. | accepted |
| FR-014 | The server can have editable human-readable profile fields, including display name and summary, separate from `federation_base_url`. | accepted |
| FR-015 | Changing `federation_base_url` is treated as a high-risk identity change and must warn that existing federated actor/activity ids become stale. | proposed |
| FR-016 | Expose a safe server descriptor at `/.well-known/agent-inbox` for peer compatibility checks. | accepted |
| FR-017 | The descriptor includes software name, version, base URL, display name, summary, federation mode/capabilities, supported schemes, and public key metadata. | proposed |
...
| FR-023 | Add actor federation visibility enum: `local`, `normal`, `discoverable`. | accepted |
...
- display name;
- summary/description;
- optional contact field if planning includes it;
- icon/avatar is future work unless trivial.
...
| C-007 | `@local` or any equivalent local-only addressing guarantee must never be federated. | accepted |
