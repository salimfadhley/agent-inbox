...
title: 'The charter and the floor say 3.14'
dependencies: []
requirement_refs:
- FR-001
- FR-006
...
# Work Package Prompt: WP01 – The charter and the floor say 3.14
...
Move the floor from Python 3.12 to 3.14 in the two places that make it true — the charter
and `pyproject.toml` — and find out what pyright says once it is analysing 3.14 semantics
rather than 3.12's.

Nothing else in this mission may start first. Until T001 lands, every other edit contradicts
the project's own governing document.
...
- **The dependencies are fine.** The full dev + `clients` + `ui` set resolved and installed
  ...
  wheels. If you find yourself adding a pin, stop — FR-006 forbids it, and the plan's
  evidence says you should not need one.
...
- **pyright reported zero errors — and that number is worthless to you.** It was still
  configured `pythonVersion = "3.12"`. T003 changes that, and T004 is where the real answer
  arrives.

## Subtasks

### T001 — Amend the charter's stated stack to 3.14+

`.kittify/charter/charter.md:50` reads:

> Languages/Frameworks: Python 3.12+ only, managed exclusively with uv…

Change `3.12+` to `3.14+`. Leave the rest of the line alone.

**This is not paperwork.** The charter is non-negotiable inside a mission, so a codebase
whose `pyproject.toml` says 3.14 while its charter says 3.12 is one where the governing
document has been quietly overruled by an implementation detail. Amend it deliberately, in
its own commit, with a message that says why.

Also check `.kittify/charter/interview/answers.yaml:15`, which carries the same sentence.
It is the record of how the charter was arrived at rather than the charter itself — update
it for consistency, and say in the commit message that you did.

### T002 — `requires-python` and the trove classifiers
...
### T003 — ruff `target-version` and pyright `pythonVersion`
...
T004 is their consequence.

### T004 — Re-run the four gates at 3.14 semantics and triage pyright
...
**WP02's T008/T009**. Note it if you see it; do not chase it, and do not skip it.

### T005 — Regenerate the lock with the same dependency set
...
expected. A package appearing or disappearing is not, and means something in T002 changed
more than the floor.
...
- [ ] The charter says 3.14+, in its own commit
...
checking the wrong language. Check the charter commit exists and is separate.
