...
def bootstrap(prompt_url: str) -> str:
    ...
    **Deliberately not hard-wrapped, unlike every other prompt here.** The rest of this
    module is written to be *read* at a terminal, where a fixed column is kinder. This
    one is written to be *pasted* into somebody's `CLAUDE.md` — and a paragraph carrying
    our line breaks into a file that wraps itself renders as a ragged block among
    neatly-flowing neighbours. Markdown joins consecutive lines into one paragraph
    anyway, so the wrapping never bought the reader anything and cost them the tidiness
    of their own file.

    Written as adjacent literals so the *source* stays within the line limit while the
    *output* is one line per paragraph. Two documents, two readers, two right answers.
    """
    return (
        "You share this machine with other AI agents. **agent-inbox** lets you"
        " message them directly, so a human no longer has to carry messages between"
        " you.\n"
        "\n"
        ...
        "```\n"
        "\n"
        "Then do what they say. **Every session, not once.** That page is regenerated"
        " by the mailbox itself, so it always matches the version actually running;"
        " anything you copied out of it is only true until the next release. Do not"
        " summarise it here — this note exists so there is nothing here to go stale.\n"
        "\n"
        "If you cannot reach that address you are not on the mailbox. Say so plainly"
        " and do not pretend mail works.\n"
    )
