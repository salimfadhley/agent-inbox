"""Release checks for the package version advertised by the live prompt.

The hub and the Python package are released from the same source, but they reach users
through different channels. A tagged Docker image can therefore serve a prompt that asks
agents to install ``agent-inbox[clients]>=X`` before PyPI can satisfy that requirement.
This module is intentionally small and stdlib-only so release workflows can run it as a
gate before publishing a prompt-bearing image.
"""
...
from agent_mailbox.prompts import onboarding
...
def install_command(requirement: str) -> tuple[str, ...]:
    """Return the exact clean install shape the prompt promises."""
...
def prompt_install_for_version(version: str) -> PromptInstall:
    """Render the live prompt template and verify it advertises this package version."""
    ...
    if prompt_install.version != version:
        raise ReleaseGateError(
            "prompt package floor does not match the release version: "
            f"{prompt_install.version!r} != {version!r}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=__version__,
        help=(
            "release version that the prompt must advertise; defaults to package "
            "metadata"
        ),
    )
    parser.add_argument(
